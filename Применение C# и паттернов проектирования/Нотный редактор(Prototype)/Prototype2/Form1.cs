...
namespace Prototype2
{
    public partial class Form1 : Form
    {
            ...
            if(notes.Count != 0)
                for (int i = 0; i < noteLines.Count; i++)
                {
                    ...
                    for (int j = 0; j < noteLines[i].amountNotes; j++)
                    {
                        ...
                        else if(notes[j + 20 * i].note == "B4" && notes[j + 20 * i].duration != 0)
                            note.RotateFlip(RotateFlipType.Rotate180FlipX);
        ...
        {
            if(notes.Count > 0)
                if (e.KeyCode == Keys.Down && notes[notes.Count - 1].relativeNotePosition > 0)
                {
                    int id = notes[notes.Count - 1].id;
                    int height = notes[notes.Count - 1].height + 5;
                    int stavePosition = notes[notes.Count - 1].stavePosition;
                    int duration = notes[notes.Count - 1].duration;
                    notes.Remove(notes[notes.Count - 1]);
                    notes.Add(new Note(id, height, stavePosition, duration));
                    paintNotes();
                } else if (e.KeyCode == Keys.Up && notes[notes.Count - 1].relativeNotePosition < 60)
                {
                    int id = notes[notes.Count - 1].id;
                    int height = notes[notes.Count - 1].height - 5;
                    int stavePosition = notes[notes.Count - 1].stavePosition;
                    ...
                    notes.Add(new Note(id, height, stavePosition, duration));
                    paintNotes();
                } else if (e.KeyCode == Keys.Left && notes[notes.Count - 1].duration > 1)
                {
                    int[] durarions = new int[5] { 1, 2, 4, 8, 16 };
                    int id = notes[notes.Count - 1].id;
                    int height = notes[notes.Count - 1].height;
                    int stavePosition = notes[notes.Count - 1].stavePosition;
                    int duration = durarions[Array.IndexOf(durarions, notes[notes.Count - 1].duration) - 1];
                    notes.Remove(notes[notes.Count - 1]);
                    notes.Add(new Note(id, height, stavePosition, duration));
                    paintNotes();
                } else if (e.KeyCode == Keys.Right && notes[notes.Count - 1].duration < 16)
                {
                    int[] durarions = new int[5] { 1, 2, 4, 8, 16};
                    int id = notes[notes.Count - 1].id;
                    int height = notes[notes.Count - 1].height;
